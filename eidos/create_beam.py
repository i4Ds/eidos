...
def main(argv):
    ...
    parser.add_argument(
        "-c",
        "--coeff",
        help="Which coefficients to use: mh for MeerKAT holography, me for MeerKAT EM simulation and vh for VLA holography?",
        type=str,
        default="mh",
    )
    ...
    parser.add_argument(
        "-S",
        "--Stokes",
        help="If provided output will be in Stokes, i. e. Mueller, formalism, instead of the default Jones formalism. Specify 'I', 'Q', 'U', 'V' for the Stokes beams, or 'M' to get the full Mueller matrix. If you give 'IQ' instead, the leakage from Q to I will be provided, and so on for any other combination of Stokes parameters.",
        type=str,
        required=False,
    )
    ...
    if len(args.freq) == 1:
        ...
        print("Do `eidos -h` to see how to input the frequency parameters")
