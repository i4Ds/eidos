...
class Zernike(object):
    ...
    def __init__(
        ...
        if "jones" in mode:
            self.jones_images(data)

    def jones_images(self, data):
        self.coeffs_J = self.coeffs_trunc_J = np.zeros(
            (data.shape[0], data.shape[1], self.Nmodes),
            dtype=np.complex128,
        )
        self.recon_full_J = self.recon_trunc_J = np.zeros(
            data.shape,
            dtype=np.complex128,
        )
        if "recon" in self.mode:
            self.recon = np.zeros(
                (2, 2, self.npix, self.npix),
                dtype=np.complex128,
            )
    ...
    def zernike_rad(self, m, n, rho):
        ...
        return sum(
            pre_fac(k) * rho ** (n - 2.0 * k) for k in np.arange((n - m) / 2 + 1)
        )
    ...
    def zernike(self, m, n, rho, phi):
        ...
        """
        if m > 0:
            return self.zernike_rad(m, n, rho) * np.cos(m * phi)
        if m < 0:
            return self.zernike_rad(-m, n, rho) * np.sin(-m * phi)
    ...
    def unit_disk(self, npix=None):
        ...
        grid = (np.indices((nx, ny), dtype=np.float64) - nx / 2) / (
            nx * 1.0 / 2
        )  # create unit grid [-1,1]
    ...
    def decompose(self):
        ...
        N = self.Nmodes
        basis = [
            self.zernikel(i, self.grid_rho, self.grid_phi) * self.grid_mask
            for i in range(N)
        ]

        # Calculate covariance between all Zernike polynomials
        self.cov_mat = np.array(
            [[np.sum(zerni * zernj) for zerni in basis] for zernj in basis]
        )
    ...
    def truncate(self, thresh):
        ...
        cutoff = int(np.round(Ncoeff * thresh / 100.0))
    ...
    def reconstruct(self):
        ...
        if "both" in self.mode:
            self.decompose()
            self.recon_full = np.sum(
                val * self.zernikel(i, self.grid_rho, self.grid_phi) * self.grid_mask
                for (i, val) in enumerate(self.coeffs)
            )
            self.res_full = (abs(self.img) - abs(self.recon_full)) * self.grid_mask
            self.truncate(self.thresh)
            self.recon_trunc = np.sum(
                val * self.zernikel(i, self.grid_rho, self.grid_phi) * self.grid_mask
                for (i, val) in enumerate(self.coeffs_trunc)
            )
